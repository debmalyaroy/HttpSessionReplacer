...
public class HelpersToInject {
    ...
    private static boolean $$isServlet3; // NOSONAR Prefer this naming
                                         // convention
                                         // here
    ...
    public final static class FilterHelpers {
         ...
         * javax.servlet.Filter implementations modified by SessionAgent. The
         * method wraps javax.servlet.ServletRequest in HttpRequestWrapper.
         * <p>
         * The method will wrap request at most once per request and will only
         * wrap instances of {@link HttpServletRequest}.
         * </p>
         *
         * @param request
         ...
         * This method is called from SessionFilter or from instrumented
         * javax.servlet.Filter implementations modified by SessionAgent. The
         * method retrieves response stored in HttpRequestWrapper.
         *
         * @param request
         *            request received by filter
         * @param response
        ...
        /**
         * Commits request and stores session in repository. This method is
         * called from the filter. The commit is only done if the filter is the
         * one that wrapped the request into HttpRequestWrapper.
         * <p>
         * The logic to check if the caller filter is the one that wrapped
         * request is based on requirement that original request and the one
         * used by filter are different and that original request is not
         * HttpRequestWrapper .
         *
         * @param request
        ...
        /**
         * Called to send debug information from instrumented call. Uses
         * standard out for output.
         *
         * @param string
    ...
    static class ListenerHelpers {
         ...
         * inside Servlet 2.5 containers. Its role is to collect session
         * listeners so they can be invoked by the library when it manages
         * sessions.
         *
         * @param listener
    ...
    static class ServletContextHelpers {
         ...
         * {@link ServletContext}. If object argument is a
         * {@link ServletContext} and listener argument contains
         * {@link HttpSessionListener} or {@link HttpSessionAttributeListener},
         * the method will add them to list of known listeners associated to
         * {@link ServletContext}
         *
         * @param object
        ...
        public static void $$onAddListener(Object object, Object listener) {
            if (!(object instanceof ServletContext)) {
                $$error("Tried registering listener %s for object %s but object was not ServletContext", listener,
                        object);
                return;
            }
    ...
    /**
     * Called to send error information from instrumented call. Uses STDERR for
     * output.
     *
     * @param format
     *            String.format message format
    ...
    private static boolean $$isServlet3() {
        try {
            ...
            return true;
        } catch (NoSuchMethodException e) { // NOSONAR no method, so no servlet
                                            // 3
            return false;
        }
    }
