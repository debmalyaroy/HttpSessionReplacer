package com.test.session.connection.api;
...
public interface RedisConnector {
    ...
    /**
     * See redis.clients.jedis.BinaryJedisCommands#hsetnx(byte[], byte[],
     * byte[])
     *
     * @param key
     *            key as byte array
    ...
    /**
     * Dissociates jedis connection from the current thread and returns it to
     * the pool.
     */
     ...
     *
     * @param key
     *            key as byte array used when performing transaction on cluster
     *            to fix the node where transaction occurs.
     * @param transaction
     ...
     */
    default boolean isRedisException(Exception e) {
     ...
     */
    static interface RedisPubSub {

        void onPMessage(byte[] pattern, byte[] channelBuf, byte[] message);

        void link(BinaryJedisPubSub actual);

        BinaryJedisPubSub getLinked();

        void start(RedisConnector redis);

        void close(RedisConnector redis);
    }
     ...
     *
     */
    static interface TransactionFacade {
         ...
         * @return
         */
        ResponseFacade<Set<byte[]>> smembers(byte[] key);
    }
     ...
     * The transaction is sequence of redis commands executed atomically.
     * <p>
