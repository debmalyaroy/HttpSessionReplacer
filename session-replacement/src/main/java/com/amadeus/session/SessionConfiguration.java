...
public class SessionConfiguration implements Serializable {
    ...
    public enum ReplicationTrigger {
        /**
         * Session data is replicated on set of the attribute and when an
         * attribute retrieved via Attribute contains a non-primitive type. This
         * means that the get of an attribute of the well-known Java type such
         * as Boolean, Character, Number (Double, Float, Integer, Long), doesn't
         * trigger replication to repository, but getAttribute operation for
         * other types of attribute will trigger update in the repository.
         */
         ...
         * prevents unnecessary replication and can benefit overall performance,
         * but is inherently unsafe as attributes that were changed after the
         * get, but where never
         */
        SET(false);
        ...
        /**
         * Returns <code>true</code> if session should be replicated on
        ...
        /**
         * Checks passed value against allowed values in enumeration. If it is
         * not valid, returns default value and logs error.
         *
    ...
    /**
     * List of attributes that must be retrieved from repository (i.e. they
     * can't be cached locally).
     */
     ...
     * case of webapps, namespace can be defined using servlet init parameter,
     * or if not present, the context name of the webapp is used as namespace.
     */
    ...
    /**
     * ServletContext parameter or system property activating sharing of
     * instances of session during concurrent requests.
     */
    ...
    /**
     * Specifies key to be used for encryption. When present activates
     * encryption automatically. If key specifies a URL, key will be loaded from
     * specified address. Otherwise it is treated literally.
     */
    ...
    public SessionConfiguration() {
        ...
        String replicationValue = getPropertySecured(SESSION_REPLICATION_TRIGGER,
                DEFAULT_REPLICATION_TRIGGER.toString());
    ...
    private void initMaxInactiveInterval(AttributeProvider provider) {
        ...
        if (nonEmpty(val)) {
            try {
                ...
                logger.warn("`{}` configuration attribute was not an integer: {} for source {}",
                        DEFAULT_SESSION_TIMEOUT, val, provider.source());
            }
        }
    }
    ...
    /**
     * Sets set of keys that should not be cached locally by extracting keys
     * from comma-separated list provided as parameter.
     *
    ...
    /**
     * Sets whether session information should be stored in logging systems
    ...
    /**
     * Returns <code>true</code> if sessions are sticky. Sticky sessions should
     * be expired on the last node that used them.
     *
    ...
    /**
     * Returns <code>true</code> if session id is expected to be suffixed by
     * !timestamp.
     *
    ...
    /**
     * Add timestamp suffix to session id if set to true. See
     * {@link #isTimestampSufix()}.
     *
     * @param timestamp
     *            <code>true</code> if session is expected to be suffixed by
     *            timestamp
     */
     ...
     *            <code>true</code> if concurrent threads can access same
     *            session
     */
     ...
     * @return value of the attribute, or property, or default value if it was
     *         not defined or if access is protected
     */
     ...
     *            <code>true</code> if distribution/replication should be used
     *            even when web app is marked as non-distributable.
     */
    ...
    /**
     * Controls if commit of attributes should be done on all concurrent
     * requests to session.
     *
    ...
    /**
     * Returns <code>true</code> if session should be encrypted before storing
     * in repository.
     *
     * @return <code>true</code> if session should be encrypted before storing
     *         in repository
     */
     ...
     * If set to <code>true</code>, encryption key must be set also.
     *
    ...
    public String toString() {
        ...
        builder.append("SessionConfiguration [namespace=").append(namespace).append(", node=").append(node)
                ...
                .append(repositoryFactory).append(", sessionTracking=").append(sessionTracking)
                .append(", encryptionKey=").append(encryptionKey).append(", nonCacheable=").append(nonCacheable)
                .append(", replicationTrigger=").append(replicationTrigger).append(", attributes=").append(attributes)
                .append(", commitOnAllConcurrent=").append(commitOnAllConcurrent).append(", timestamp=")
                .append(timestampSufix).append("]");
