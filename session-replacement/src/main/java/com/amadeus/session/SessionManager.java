...
public class SessionManager implements Closeable {
     ...
     *            {@link SessionManager} from different namespaces request
     *            access to session with same id, repository should give
     *            different sessions (unless a special configuration overrides
     *            this).
     * @param tracking
    ...
    public SessionManager(ExecutorFacade executors, SessionFactory factory, SessionRepository repository,
            SessionTracking tracking, SessionNotifier notifier, SessionConfiguration configuration,
    ...
    /**
     * Starts monitoring this session manager. The method will expose all
     * metrics through JMX.
     */
    ...
    /**
     * Retrieves id from request. The forceId parameter is used to force usage
     * of certain id (e.g. when forwarding request from one web app to another).
     *
     * @param request
     ...
     *
     * @param timer
     *            if not null, the time to execute task will be measured and
     *            stored under timer with given name
     * @param task
     *            the task to run
    ...
    /**
     * Schedules the tasks to execute with a {@link ScheduledExecutorService}
    ...
    /**
     * Calls {@link Committer} for the passed {@link RepositoryBackedSession}
     * and measures time of execution.
     *
     * @param session
    ...
    /**
     * Serialiazer/deserializer to use when storing to repository. Serialaizer
     * and deserializers can be configured.
     *
    ...
    final class RunnableWithTimer implements Runnable {
        ...
        @Override
        public void run() {
            // Following variable is used in try/finally to measure execution
            // time
            try (Timer.Context ctx = timer.time()) { // NOSONAR
                ...
                logger.warn("Unexpected exception occured in time measured session related task in category " + timer,
                        e);
            }
        }
    }
