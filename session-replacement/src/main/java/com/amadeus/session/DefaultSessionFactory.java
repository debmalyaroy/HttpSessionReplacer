...
public class DefaultSessionFactory implements SessionFactory {
    ...
    public RepositoryBackedSession build(SessionData sessionData) {
        if (useCached) {
            ...
            if (session == null) {
                ...
                if (oldSession != null) {
                    // If we had race condition, and someone added new session
                    // to cache,
                    // use what is in cache
                    session = wrapSession(oldSession);
                }
            } else {
                // Wrap cached session to avoid leaking of commit from one to
                // another.
    ...
    /**
     * Wraps old session object into a new one. This method can be overridden in
     * a subclass to provide wrapping of sessions in different implementations
     * (e.g. HttpSession).
     *
     * @param oldSession
     *            session to wrap
                ...
                if (cachedSessionData != sessionData) { // NOSONAR
                    synchronized (cachedSessionData) {
                        if (!cachedSessionData.isIdChanged()) {
                            ...
                            logger.warn(
                                    "Session id was already switched for session in cache: {}, new session should be: {}",
