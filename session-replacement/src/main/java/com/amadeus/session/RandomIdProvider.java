...
public class RandomIdProvider implements SessionIdProvider {
    ...
    private char[] encode(byte[] data) {
        ...
        for (int i = 0, index = 0; i < data.length; i++, index += CHARACTERS_IN_BLOCK) {
            ...
            val <<= MULTIPLY_BY_256;
            i++; // NOSONAR each loop is actually i+3, and we increment counter
                 // inside loop
            if (i < data.length) {
            ...
            out[index + 3] = alphabet[(quad ? (val & MASK_6_BITS) : FILLER_CHARACTER_INDEX)]; // NOSONAR
                                                                                              // 3
                                                                                              // is
                                                                                              // not
                                                                                              // magic!
            val >>= DIVIDE_BY_64;
            out[index + 2] = alphabet[(trip ? (val & MASK_6_BITS) : FILLER_CHARACTER_INDEX)]; // NOSONAR
                                                                                              // 2
                                                                                              // is
                                                                                              // not
                                                                                              // magic!
