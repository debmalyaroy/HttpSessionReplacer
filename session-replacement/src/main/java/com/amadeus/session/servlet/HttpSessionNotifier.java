 ...
 */
public class HttpSessionNotifier implements SessionNotifier {
    private ServletContextDescriptor descriptor;

    public HttpSessionNotifier(ServletContextDescriptor descriptor) {
     ...
     * (non-Javadoc)
     *
     * @see com.amadeus.session.http.SessionNotifier#sessionCreated(com.amadeus.
     ...
     * <p>
     * If the added attribute <code>value</code> is a
    ...
    /**
     * Notifies listeners that attribute was replaced. See
    ...
    public void attributeReplaced(RepositoryBackedSession session, String key, Object replacedValue) {
        if (session instanceof HttpSession && replacedValue instanceof HttpSessionBindingListener) {
            ((HttpSessionBindingListener) replacedValue)
                    .valueUnbound(new HttpSessionBindingEvent((HttpSession) session, key));
        }
    ...
    /**
     * Notifies listeners that attribute was removed. See
     * {@link SessionNotifier}
    ...
    public void attributeRemoved(RepositoryBackedSession session, String key, Object removedValue) {
        if (session instanceof HttpSession && removedValue instanceof HttpSessionBindingListener) {
            ((HttpSessionBindingListener) removedValue)
                    .valueUnbound(new HttpSessionBindingEvent((HttpSession) session, key));
        }
     ...
     * (non-Javadoc)
     *
     * @see com.amadeus.session.http.SessionNotifier#attributeBeingStored(com.
     ...
     *
     * @see
     * com.amadeus.session.http.SessionNotifier#attributeHasBeenRestored(com.
     * amadeus.session.RepositoryBackedSession, java.lang.String,
     * java.lang.Object)
     */
    @Override
     ...
     *
     * @see
     * com.amadeus.session.http.SessionNotifier#sessionDestroyed(com.amadeus.
     * session.RepositoryBackedSession, boolean)
     */
    @Override
    public void sessionDestroyed(RepositoryBackedSession session, boolean shutdown) {
        if (session instanceof HttpSession) {
            // We notify all session attribute listeners that each attribute is
            // removed
