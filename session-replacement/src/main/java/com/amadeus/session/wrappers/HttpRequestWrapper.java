 ...
 */
public class HttpRequestWrapper extends SlingHttpServletRequestWrapper implements RequestWithSession {
    ...
    public HttpRequestWrapper(HttpServletRequest req, ServletContext servletContext) {
        super((SlingHttpServletRequest) req);
    ...
    public RepositoryBackedHttpSession getSession(boolean create) {
        if (embeddedRequest != null) {
            // If there is embedded HttpRequestWrapper, create session there if
            // needed
    ...
    public RepositoryBackedHttpSession getRepositoryBackedSession(boolean create) {
        ...
        // Only propagate session if this is the "outer" session, i.e. the one
        // that
    ...
    /**
     * Get wrapped request if it is an {@link HttpRequestWrapper}. Returns
     * <code>null</code> if wrapped wrapped request was not HttpRequestWrapper.
     * 
     * @return
     */
